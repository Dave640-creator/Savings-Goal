...
import '../widgets/goal_details_sheet.dart';
    ...
    return Consumer<AppState>(
      builder: (context, state, _) => Scaffold(
        ...
        body: state.goals.isEmpty
            ...
            : Column(
                children: [
                  _GoalsPieChart(goals: state.activeGoals),
                  Expanded(
                    child: ListView(
                      ...
                      children: [
                        ...
                        if (state.activeGoals.isNotEmpty) ...[
                          ...
                          ...state.activeGoals.map((goal) => _GoalCard(
                                ...
                                onEdit: () => _showGoalForm(context, state,
                                    existing: goal),
                                onDelete: () =>
                                    _confirmDelete(context, state, goal),
                              )),
                        ],
                        ...
                        if (state.completedGoals.isNotEmpty) ...[
                          ...
                          ...state.completedGoals.map((goal) => _GoalCard(
                                goal: goal,
                                onEdit: () => _showGoalForm(context, state,
                                    existing: goal),
                                onDelete: () =>
              ...
              Row(
                children: [
                  ...
                  PopupMenuButton(
                    ...
                    onSelected: (val) {
                      if (val == 'edit') onEdit();
                      if (val == 'details') _showDetails(context);
...
class _GoalFormSheetState extends State<_GoalFormSheet> {
  ...
  String? _validateName(String? value) {
    ...
    final isDuplicate = widget.state.goals.any((g) =>
        g.name.toLowerCase() == name &&
        (widget.existing == null || g.id != widget.existing!.id));
    if (isDuplicate) {
  ...
  void _submit() {
    ...
    try {
      setState(() {
        ...
        _targetError = _validateTarget(_targetCtrl.text);
        _savedError = _validateSaved(_savedCtrl.text);
      });
      ...
      if (saved > target) {
        setState(() {
          _savedError = "Saved amount cannot exceed target";
        });

        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Saved cannot be greater than target')),
        );

        return;
      }

      // Step 3: proceed save (okay na lahat)
